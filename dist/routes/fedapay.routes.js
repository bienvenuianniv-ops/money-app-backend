 ...
 */
router.get("/fedapay/methods/:countryCode", auth_1.requireAuth, (req, res) => {
    const methods = fedapay_service_1.FedaPayService.getPaymentMethods(req.params.countryCode);
    if (methods.length === 0) {
 ...
 * }
 */
router.post("/fedapay/deposit", auth_1.requireAuth, async (req, res, next) => {
    try {
        ...
        const result = await fedapay_service_1.fedapay.initiateDeposit({
            amount,
            currency,
 ...
 */
router.get("/fedapay/status/:transactionId", auth_1.requireAuth, async (req, res, next) => {
    try {
        const result = await fedapay_service_1.fedapay.checkTransactionStatus(parseInt(req.params.transactionId));
 ...
 * }
 */
router.post("/fedapay/withdraw", auth_1.requireAuth, async (req, res, next) => {
    try {
        ...
        const result = await fedapay_service_1.fedapay.initiateWithdraw({
            amount,
            currency,
            phone,
            firstName,
...
router.post("/webhooks/fedapay", async (req, res) => {
    try {
        ...
        if (event.status === "approved" && event.id) {
            ...
            if (tx && tx.toWallet) {
                await db_1.prisma.$transaction(async (db) => {
...
//# sourceMappingURL=fedapay.routes.js.map