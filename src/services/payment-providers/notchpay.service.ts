...
export interface NotchpayConfig {
  publicKey:  string;  // process.env.NOTCHPAY_PUBLIC_KEY
  privateKey: string;  // process.env.NOTCHPAY_PRIVATE_KEY
  hashKey:    string;  // process.env.NOTCHPAY_HASH_KEY
  callbackUrl: string; // URL de callback après paiement
  notifyUrl:   string; // URL webhook
}
...
export class NotchpayService {
  ...
  async initiateDeposit(params: {
    ...
    phone: string;
    name: string;
    reference: string;      // ID unique de votre côté
    description: string;
  ...
  async initiateWithdraw(params: {
    amount:    number;
    currency:  string;
    phone:     string;      // numéro Mobile Money avec indicatif
    channel:   string;      // cm.mtn | cm.orange | sn.wave | sn.orange | ci.mtn | ci.orange
    name:      string;
    email?:    string;
    reference: string;
    description?: string;
