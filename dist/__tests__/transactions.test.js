...
async function register(phone) {
    const res = await (0, supertest_1.default)(app_1.default).post("/auth/register").send({ phone, pin });
...
async function login(phone) {
    const res = await (0, supertest_1.default)(app_1.default).post("/auth/login").send({ phone, pin });
...
async function getWallet(token) {
    const res = await (0, supertest_1.default)(app_1.default).get("/wallet").set("Authorization", `Bearer ${token}`);
    expect(res.status).toBe(200);
    expect(res.body.ok).toBe(true);
    return res.body.wallet;
}
async function getSystemWallet() {
    const res = await (0, supertest_1.default)(app_1.default).get("/dev/system-wallet");
    expect(res.status).toBe(200);
    expect(res.body.ok).toBe(true);
    return res.body.wallet;
}
describe("Transactions", () => {
    beforeAll(async () => {
        await (0, supertest_1.default)(app_1.default).post("/dev/bootstrap-system");
    });
    afterAll(async () => {
    ...
    test("deposit augmente user et baisse system", async () => {
        ...
        const dep = await (0, supertest_1.default)(app_1.default)
    ...
    test("withdraw baisse user et augmente system", async () => {
        ...
        const wBefore = await getWallet(token);
        const sBefore = await getSystemWallet();
        const wd = await (0, supertest_1.default)(app_1.default)
    ...
    test("transfer bouge les 2 wallets", async () => {
        ...
        const sBefore = await getSystemWallet();
        const tr = await (0, supertest_1.default)(app_1.default)
